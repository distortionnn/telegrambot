...
async def main_buttons(user: User):
    ...
    if user.subscription != "none":
        ...
        Butt_main.add(types.KeyboardButton(e.emojize(f"Продлить :money_bag:")),types.KeyboardButton(e.emojize(f"Подключить VPN :gear:")))
        Butt_main.add(types.KeyboardButton(e.emojize(f"FAQ :thinking_face:")))
...
async def admin_buttons():
    ...
    Butt_admin.add(types.KeyboardButton(e.emojize(f"Сервера :desktop_computer:")), types.KeyboardButton(e.emojize(f"Статичные")))
    Butt_admin.add(types.KeyboardButton(e.emojize("Отправить уведомление :speech_balloon:")))
...
async def admin_buttons_server():
    Butt_admin = types.ReplyKeyboardMarkup(resize_keyboard=True)
    Butt_admin.add(types.KeyboardButton(e.emojize(f"Все сервера :bar_chart:")))
    Butt_admin.add(types.KeyboardButton(e.emojize(f"Добавить сервер :plus:")))
    Butt_admin.add(types.KeyboardButton(e.emojize("Удалить сервер :fire:")))
    Butt_admin.add(types.KeyboardButton(e.emojize("Назад :right_arrow_curving_left:")))
    return Butt_admin
async  def admin_buttons_server_back():
    Butt_admin = types.ReplyKeyboardMarkup(resize_keyboard=True)
    Butt_admin.add(types.KeyboardButton(e.emojize(f"Вернутья :right_arrow_curving_left:")))
    return Butt_admin
